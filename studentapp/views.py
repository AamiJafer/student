...
def addStudent(request):
    if request.method=="POST":
        ...
        stud=studentRegister(name=n,address=addr,age=a,email=mailid,joiningDate=jod,qualification=q,gender=gen,mobile=m,password=passwrd)
        stud.save()
        subject="Application for Python Developer"
        message=f'Your Username: {n} \n Password: {passwrd}'
        send_mail(subject,message,settings.EMAIL_HOST_USER,[mailid])
